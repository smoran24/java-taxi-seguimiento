package com.tutomas.java_taxi_seguimiento.inicial;
...
public class CommandLineInit implements CommandLineRunner {
    ...
    private final String url = "https://vite-taxi-seguimiento.vercel.app/taxiapp/send-coordenada";
    ...
    private void enviarCoordenada(Coordenada coordenada) {
        ...
        ResponseEntity<Coordenada> responseEntity = restTemplate.postForEntity(url, requestEntity, Coordenada.class); //envia peticion post a esa url

        if(responseEntity.getStatusCode().is2xxSuccessful()){ //por si retorna error
